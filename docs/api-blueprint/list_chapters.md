# Lấy Danh Sách Chủ Đề (List Chapters)
...
- **Description**: Tra cứu và lấy danh sách chủ đề theo cơ chế phân trang (Pagination), hỗ trợ lọc theo từ khóa (title), trạng thái, người tạo.
...
- **HTTP Method**: `GET`
- **URL**: `/api/v1/content-builder/chapters`
...
| `keyword` | String | No | Từ khóa tìm kiếm tương đối trong tiêu đề chủ đề (`title ILIKE %keyword%`) | Tối đa 100 ký tự |
...
|---|---|---|
| `content` | Array<Object> | Danh sách tóm tắt các chủ đề trong trang hiện tại |
...
*(Mỗi phần tử trong mảng `content` trả về các thông tin chung: `id`, `title`, `status`, `createdBy`, `createdAt`, `updatedAt`)*.
...
| `AUTH-403` | 403 Forbidden | Người dùng không có quyền xem danh sách chủ đề | You do not have permission to view chapters. |
...
1. **Controller Layer**:
   - Nhận GET request tại `/api/v1/content-builder/chapters`.
   - Tạo đối tượng `Pageable` từ `page`, `size`, `sort`.
   - Gọi Service Layer: `listChapters(filterCriteria, pageable)`.
...
3. **Repository Layer**:
   - Thực hiện lệnh SELECT COUNT(*) và SELECT phân trang trên bảng `chapters`.
...
- **Operation 1**:
  ...
  - **Expected Result**: Trang dữ liệu chủ đề và tổng số bản ghi.

---

## Part 4 — Operational Notes

- **Performance**: Đảm bảo các cột được lọc thường xuyên (`status`, `created_by`) đã được tạo Index trên bảng `chapters`.
